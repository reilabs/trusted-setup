...
var last phase2.Coordinator

func testStartServer(t *testing.T) {
	var err error

	config, err = server_config.NewConfig("resources/config.json")
	...
	last = phase2.FromPhase1(p1, ccs, beacon)

	manager := contributors_manager.New()
	service := ceremony_service.New(config.CeremonyName, last, manager)
...
func testRunContributions(t *testing.T) {
	...
	for i := 0; i < clientsCount; i++ {
		assert.NoError(t, client.ConnectAndContribute(config.Host, strconv.Itoa(config.Port)))
	}
	...
	for i := 0; i < clientsCount; i++ {
		...
		go func() {
			defer wg.Done()
			err := client.ConnectAndContribute(config.Host, strconv.Itoa(config.Port))
			errCh <- err
		}()
	}
